...
WITH donaciones AS (
    ...
    {% if is_incremental() %}
        -- Filtramos los datos crudos primero para no hacer JOINs pesados de la historia completa
        WHERE donation_date > (SELECT MAX(fecha_key) FROM {{ this }})
    {% endif %}
),
